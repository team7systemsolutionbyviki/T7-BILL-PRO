...
function toggleMenuNotAvailableToday(productId) {
    ...
    if (products[idx].menuNotAvailableDate === today) {
        products[idx].menuNotAvailableDate = null;
    } else {
        products[idx].menuNotAvailableDate = today;
    }
    localStorage.setItem('mediflow_products', JSON.stringify(products));
    syncToCloud('products', products);
    renderMenuCard();
...
function renderMenuCard(query) {
    ...
    sortedKeys.forEach(key => {
        ...
        groups[key].forEach(p => {
            ...
            const showStock = p.menuShowStock === true;
...
function saveTableList() {
    localStorage.setItem('mediflow_tables', JSON.stringify(tableList));
    if (typeof syncToCloud === 'function') syncToCloud('tables', tableList);
}
...
function renderTableManagement() {
    ...
    grid.innerHTML = tableList.map((t, idx) => {
        ...
        return `
            <div style="background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 12px; padding: 1.25rem; display: flex; flex-direction: column; gap: 0.5rem; box-shadow: 0 2px 8px rgba(0,0,0,0.06);">
                ...
                <div style="font-size: 0.8rem; color: var(--text-muted);">${escapeHtml(t.zone || 'General')} ${t.capacity > 0 ? '· ' + t.capacity + ' seats' : ''}</div>
                <div style="font-size: 0.85rem; color: var(--success-color); font-weight: 600;">Today: ${settings.currency || '₹'}${todayRevenue.toFixed(2)} (${tableSales.length} bills)</div>
    ...
    if (tbody) {
        if (todaySales.length === 0) {
            ...
            tbody.innerHTML = Object.entries(tMap).map(([name, v]) => {
                grandBills += v.bills; grandRev += v.rev;
...
function saveTableEntry() {
    ...
    if (idx !== null && tableList[idx]) {
        ...
        tableList.push({ id: 'TBL' + Date.now(), name, capacity, zone, status: 'Available' });
    }
    saveTableList(); closeAddTableModal(); renderTableManagement();
}

function editTableEntry(idx) { openAddTableModal(idx); }
